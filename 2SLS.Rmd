...
```{r}
corrvalue <- 0.5 # los resultados serán diferentes según cambie este valor
...
Al estimar con OLS esta situación, es esperable que entregue un estimador sesgado, es decir, que su valor esperado no esté en el valor real (por construcción $\beta = 5$). Para evaluar esto, correremos 400 simulaciones del caso recién descrito, estimando cada vez $(\beta_{OLS})$. Observa que ocurre en [el gráfico.](https://a2ic.shinyapps.io/grafico_errores_sesgo/#section-gráfico-interactivo) ¿Cuál es la relación entre la correlación de los errores simulados y el sesgo?
...
ggplot(as.data.frame(B_OLS)) + 
  ...
  labs(x="Estimador OLS", #agrega etiqueta del eje X
...
Esta vez, nos concentraremos en la varianza de nuestro estimador. La simulación genera 400 iteraciones de una mismo escenario. Al mirar el gráfico se puede ver que, aunque la varianza cambie, el promedio o valor medio siempre está en torno al valor 5, que es, el valor definido del parámetro $\beta = 5$. ¿El método de VI logra corregir el sesgo de endogeneidad? ¿Qué ocurrirá si se aumenta el tamaño de la muestra en cada escenario?
...
¿La estimación es insesgada? ¿Es eficiente? ¿Crees que funciona utilizar una simulación de un instrumento?
